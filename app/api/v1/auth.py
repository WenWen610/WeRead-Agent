...
from app.features.auth.store import auth_store
...
async def get_current_user(
    ...
    try:
        ...
        user = await auth_store.get_user(user_id_int)
        if user is None:
...
async def register_user(request: Request, user_data: UserCreate):
    ...
    try:
        ...
        if await auth_store.get_user_by_email(sanitized_email):
            raise HTTPException(status_code=400, detail="Email already registered")

        # Create user
        user = await auth_store.create_user(email=sanitized_email, password=User.hash_password(password))
...
async def login(
    ...
    try:
        ...
        user = await auth_store.get_user_by_email(username)
...
async def create_session(user: User = Depends(get_current_user)):
    """Create a new authenticated app session for the current user.

    Args:
        user: The authenticated user

    Returns:
        SessionResponse: The session ID, display name, and access token
    """
    try:
        ...
        session = await auth_store.create_session(session_id, user.id)
...
async def update_session_name(
    ...
    try:
        ...
        session = await auth_store.update_session_name(sanitized_session_id, sanitized_name)
...
async def delete_session(session_id: str, current_session: Session = Depends(get_current_session)):
    ...
    try:
        ...
        # Delete the session
        await auth_store.delete_session(sanitized_session_id)
...
async def get_user_sessions(user: User = Depends(get_current_user)):
    """Get all authenticated app sessions for the current user.

    Args:
        user: The authenticated user

    Returns:
        List[SessionResponse]: List of authenticated app sessions
    """
    try:
        sessions = await auth_store.get_user_sessions(user.id)
        return [
            SessionResponse(
